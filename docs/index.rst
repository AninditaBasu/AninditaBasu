...
Things on Internet Archive are called `items`. A song, a book, or a video is an item. Every item can have one or more files. For example, an item called `Euclid's Geometry` can have a `PDF` file, an `HTML` file, and a `TXT` file. Items can be placed in collections. For example, a collection called `European Libraries` can contain several items, one of which can be `Euclid's Geometry`. An item can belong to more than one collection.



Item
****
An item is a collection of files. Every item has a unique identifier
